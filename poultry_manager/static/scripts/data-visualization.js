document.addEventListener('DOMContentLoaded', function() {
    ...
    fetchData('/api/production-data').then(data => {
        new Chart(document.getElementById('eggCollectionChart').getContext('2d'), {
            type: 'line',
            data: {
                labels: data.labels,
                datasets: [{
    ...
    fetchData('/api/health-record-data').then(data => {
    ...
    }).catch(error => console.error('Error fetching health record data:', error));

    // Initialize the Flock Distribution Chart
    fetchData('/api/flock-data').then(data => {
